 ...
 */
public class HJ16_ShoppingCart_DP {
  ...
  public static int makeHimHigh(int money, ArrayList<Good> goods){
    ...
    int[][] dp = new int[goods.size()][money + 1];
    ...
    //开始执行背包
    int ans = p(keys, teams, 0, money, dp);
    return ans;
  }

  //递归的方法在40的时候超时
  public static int p(int[] keys, HashMap<Integer, ArrayList<Good>> teams, int i, int rest, int[][] dp){
    if(rest < 0) return -1;
...
//    int ans = 0;
    if(len == 1){// a : a 或者 不要
      //不要主物品
      int p1 = p(keys, teams, i + 1 , rest, dp);
      //要
      int p2 = -1;
      int next = p(keys, teams, i + 1, rest - maingood.price, dp);
      if(next != -1){
      ...
      //要a 不要 b
      int p2 = -1;
      int next2 = p(keys, teams, i + 1, rest - maingood.price,dp);
      if(next2 != -1){
        p2 = maingood.price * maingood.importance + next2;
      }
      //要a和b
      int p3 = -1;
      int next3 = p(keys, teams, i + 1, rest - maingood.price - b.price,dp);
      if(next3 != -1){
        p3 = maingood.price * maingood.importance + b.price * b.importance + next3;
      }
      dp[i][rest]= Math.max(p1, Math.max(p2, p3));
      ...
      //要ac
      int p4 = -1;
      int next4 = p(keys, teams, i + 1, rest - maingood.price - c.price,dp);
      if(next4 != -1){
      ...
      //abc
      int p5 = -1;
      int next5 = p(keys, teams, i + 1, rest- maingood.price - b.price - c.price,dp);
      if(next5 != -1){
        p5 = maingood.price * maingood.importance + c.price * c.importance + b.price * b.importance + next5;
      }
      dp[i][rest] = Math.max(Math.max(Math.max(p1,p2),Math.max(p3, p4)), p5);
    }
    return dp[i][rest];
  }
