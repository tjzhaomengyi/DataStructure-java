...
public class MinString {
  ...
  String minWindow(String s,String t) {
    ...
    Map<Character, Integer> need = new HashMap<Character, Integer>();//存储当前窗口中字符及其个数
    Map<Character,Integer> window = new HashMap<Character, Integer>();//存储字符串t中字符个数
    ...
    while(right<s.length()){
      ...
      while(valid == need.size()){//此时window已经包含了t的所有字符，但是要收缩left求最小字符
        ...
        if(need.containsKey(remove)){//如果t对应need包含左侧要删除的字符，并且window和need中该字符相等，那么就可以缩小窗口
          ...
          if(window.get(remove).equals(need.get(remove))){//如果是window这个值和need[t]中的值一样，然后还减少了，就-1，说明windows中删除了一个need中的值
            valid--;
          }
