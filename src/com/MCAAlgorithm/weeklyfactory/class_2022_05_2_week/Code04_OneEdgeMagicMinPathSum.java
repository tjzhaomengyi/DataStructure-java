...
public class Code04_OneEdgeMagicMinPathSum {
	...
	// 同理bcd三个点也这么处理，如果到达了b，我们就可以到a'和c'，如果到了b'，在这个例子中哪个都走不了了。结论：一旦走了魔法路径到了x',说明适用了魔法路径，
	// 不能在用魔法了，并且只能通过实边来都后米都带'的点。虚边只能走一次，到了虚点后后面只能走到虚点的实边
	//                     b
	//   a    .........             ........c
	//                     d
	//如果从x可以到abcefg，到了x‘只能通过实边a'b'c',不能到abc了
	// 时间复杂度O(N * logN)
	// N <= 2 * 10^5
